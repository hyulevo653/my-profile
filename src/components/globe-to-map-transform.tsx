...
// Simple helper: choose between globe (orthographic) and map (equirectangular)
// Avoids using `geoProjectionMutator` which may not be available in the runtime.
function createInterpolatedProjection(t: number, rotation: number[], translation: number[], width: number, height: number) {
  const alpha = Math.pow(t, 0.5)
  const scaleVal = d3.scaleLinear().domain([0, 1]).range([200, 120])(alpha)
...
export function GlobeToMapTransform() {
  ...
  useEffect(() => {
    const loadWorldData = async () => {
      try {
        ...
        // `feature(...)` can return either a GeoJSON FeatureCollection or a single Feature
        // Narrow safely to a FeatureCollection when possible to access `.features`.
        const topoResult = feature(world as any, (world as any).objects.countries) as
          | GeoJSON.FeatureCollection<any>
          | GeoJSON.Feature<any>
        const countries = "features" in topoResult ? topoResult.features : [topoResult] as any[]
  ...
  useEffect(() => {
    ...
    const projection = createInterpolatedProjection(t, rotation, translation, width, height)
    ...
    svg
      ...
      .attr("opacity", 1.0)
      .style("visibility", function (this: any) {
